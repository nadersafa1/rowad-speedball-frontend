...
/**
 * Shared match utility functions.
 * Used by both socket-based live scoring page and REST-based dialog form.
 */

/** Calculate set wins for each registration */
export const calculateSetWins = (sets: Set[] | undefined) => {
  ...
  return sets
    ...
    .reduce(
      (acc, set) => {
        if (set.registration1Score > set.registration2Score) acc.player1++
        else if (set.registration2Score > set.registration1Score) acc.player2++
        return acc
      },
      { player1: 0, player2: 0 }
    )
...
/** Check if majority reached from sets array */
export const hasMajorityFromSets = (sets: Set[], bestOf: number): boolean => {
  const wins = calculateSetWins(sets)
  return hasMajorityReached(wins.player1, wins.player2, bestOf)
}
